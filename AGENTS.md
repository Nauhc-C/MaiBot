...
# 日志检索 CLI 使用约定
如果你在使用给 agent 用的日志检索 CLI，请按以下方式使用：
1. 先做搜索，再决定读哪段原文；不要一上来就请求全量日志正文。
2. 优先使用时间范围、精确句子、关键词、会话、阶段、上下文窗口等条件，把候选范围尽量缩小。
3. 默认应只读取索引、摘要和位置指针；只有在确认某条记录值得展开时，再显式读取原文。
4. 遇到 planner / replyer / user 相关问题时，优先利用它们之间的上下文关联，先定位同一轮候选记录，再展开需要的片段。
5. 如果 CLI 返回 JSON，优先按结构化字段继续筛选和决策，不要把长文本字段整段转抄进上下文。
